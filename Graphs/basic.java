...
public class basic{
    ...
    //bfs for cycle
    //T: O(E) S: O(E) as E no. of ele are added in que
    //V<E
    ...
    //optimised bfs
    //bfs without cycle
    //T: O(V) as que m sirf vertices hi add hote h
    //V<E 
    ...
    public static boolean isBipartite(ArrayList<Edge>[] graph, int[] vis, int src){
        ...
        if(isCycle){
            if(isBipartite)
                System.out.println("Graph is Bi-partite, it means it has an even length cycle");
            else
                System.out.println("Graph is Non Bi-partite, it means it has an odd length cycle");
        }
        else{
