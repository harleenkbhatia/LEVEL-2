public class Questions{
    ...
    public int dfs(int sr, int sc, int er, int ec, boolean[][] block, int[][] dp){
        ...
        int count=0;
        if(sc+1 <= ec && !block[sr][sc+1]) count = (count+dfs(sr, sc+1, er, ec, block,dp)%mod)%mod;
        if(sr+1 <= er && !block[sr+1][sc]) count = (count+dfs(sr+1, sc, er, ec, block,dp)%mod)%mod;
    ...
    public int FindWays(int n, int m, int[][] blocked_cells)
    {
        ...
        for(int[] b : blocked_cells) block[b[0]][b[1]] = true;
        
        if(block[1][1] || block[n-1][m-1]) return 0;
        int[][] dp = new int[n][m];
    ...
    public static int floodFill(int sr, int sc, int er, int ec, int[][] dir, String[] Sdir, int[][] vis, String psf, ArrayList<String> ans){
        if(sr==er && sc==ec) {
            ans.add(psf);
            return 1;
        }
        ...
        for(int d = 0; d < dir.length; d++){
            ...
            if(r>=0 && c>=0 && r<=er && c<=ec && vis[r][c] == 1){
                count += floodFill(r, c, er, ec, dir, Sdir, vis, psf + Sdir[d],ans);
            }
        }
        vis[sr][sc] = 1;
        return count;
    ...
    public static ArrayList<String> findPath(int[][] m, int n) {
        ...
        int res  = floodFill(0, 0, n-1, n-1, dir, Sdir, m, "", ans);
        Collections.sort(ans); //sorts the ans in lexicographical order
        return ans;
    }
