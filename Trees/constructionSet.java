...
public class constructionSet{
    ...
    public static TreeNode preOrIn(int[] pre, int[] in, int psi, int pei, int isi, int iei) {
        ...
        root.left = preOrIn(pre, in, psi + 1, pei + tel, isi, idx - 1);
    ...
    public static TreeNode buildTree(int[] preorder, int[] inorder) {
        ...
        return preOrIn(preorder, inorder, 0, preorder.length - 1, 0, inorder.length - 1);
    ...
    public TreeNode constructFromPrePost(int[] pre, int[] post) {
        int n = pre.length;
        int m = post.length;
        return constructFromPrePost(pre, 0, n-1, post, 0, m-1);
    }
    ...
    public static TreeNode deserialize_preorder(String[] arr, int[] idx) {
        ...
        int i = idx[0]++;
        int val = Integer.parseInt(arr[i]); //converts string to integer
